...
@Component
@Primary
public class MockConservationAdapter implements ConservationAdapter {
    public static final String CODE = "MOCK_CONSERVATION_TEST_ONLY";
    private final MockDeliveryScenarioResolver scenarios;
    private final ObjectMapper objectMapper;

    MockConservationAdapter(MockDeliveryScenarioResolver scenarios, ObjectMapper objectMapper) {
    ...
    @Override public AdapterResult submit(DeliveryRequest request) {
        String reference = "mock-conservation-" + request.correlationId();
    ...
    @Override public AdapterResult reconcile(ReconciliationRequest request) {
        AdapterResult.Status status = scenarios.outcome(request.contextReference(), "CONSERVATION",
        ...
        return result(status, request.remoteReference(), request.correlationId(), type,
                status == AdapterResult.Status.REJECTED ? "MOCK_CONSERVATION_REJECTED" :
                        status == AdapterResult.Status.TIMEOUT ? "MOCK_CONSERVATION_TIMEOUT" : null,
                status == AdapterResult.Status.REJECTED ? "Rifiuto conservazione simulato" :
                        status == AdapterResult.Status.TIMEOUT ? "Timeout conservazione simulato" : null);
    }
    ...
    private AdapterResult result(AdapterResult.Status status, String reference, String correlationId,
        ...
        try {
            ...
            return new AdapterResult(status, reference, objectMapper.writeValueAsBytes(value), receiptType,
                    "application/json", errorCode, errorMessage);
        } catch (Exception exception) { throw new IllegalStateException("Unable to create mock conservation receipt", exception); }
    }
}
