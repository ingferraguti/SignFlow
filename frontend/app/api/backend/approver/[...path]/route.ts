...
async function proxy(request: NextRequest, context: Context) {
  ...
  const target = new URL(`${backendUrl}/api/approver/${path.join("/")}`); target.search = request.nextUrl.search;
  const response = await fetch(target, { method: request.method,
    headers: { Authorization: `Bearer ${session.accessToken}`, "content-type": request.headers.get("content-type") ?? "application/json" },
