...
import {
  emptyReportFilters,
  fetchReportDetail, fetchReports, fetchReportSourceSystems, fetchReportWorkflow, fetchWorkflowSigners, reportStates,
  type ReportDetail, type ReportFilters, type ReportPage, type ReportWorkflow,
  type SourceSystemOption, type WorkflowSigner,
...
export function AdminReportsPanel() {
  ...
  useEffect(() => {
    Promise.all([fetchReports(emptyReportFilters()), fetchReportSourceSystems(), fetchWorkflowSigners()])
      .then(([page, loadedSources, loadedSigners]) => { setResult(page); setSources(loadedSources); setSigners(loadedSigners); })
      .catch((reason: Error) => setError(reason.message));
  }, []);

  async function search(event?: FormEvent, nextPage = 0, nextFilters = filters) {
    event?.preventDefault(); setError(undefined); setDetail(undefined); setWorkflow(undefined);
  ...
  async function showDetail(id: string) {
    setError(undefined);
    try { const [loadedDetail, loadedWorkflow] = await Promise.all([fetchReportDetail(id), fetchReportWorkflow(id)]); setDetail(loadedDetail); setWorkflow(loadedWorkflow); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Dettaglio non disponibile"); }
  }

  async function refreshDetail(id: string) {
    const [loadedDetail, loadedWorkflow, page] = await Promise.all([
      fetchReportDetail(id), fetchReportWorkflow(id), fetchReports(applied, result.page),
    ]);
    setDetail(loadedDetail); setWorkflow(loadedWorkflow); setResult(page);
  }
  ...
  return <div className="reports-layout">
    ...
    {detail && workflow ? <ReportDetailPanel detail={detail} workflow={workflow} signers={signers}
      reload={() => refreshDetail(detail.id)} close={() => { setDetail(undefined); setWorkflow(undefined); }} /> : null}
  </div>;
}
...
function ReportDetailPanel({ detail, workflow, signers, reload, close }: { detail: ReportDetail; workflow: ReportWorkflow; signers: WorkflowSigner[]; reload: () => Promise<void>; close: () => void }) {
  const { text } = useUiTexts();
