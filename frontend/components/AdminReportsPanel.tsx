...
import {
  emptyReportFilters,
  fetchReportDetail, fetchReportReview, fetchReports, fetchReportSourceSystems, fetchReportWorkflow,
  fetchWorkflowApprovers, fetchWorkflowSigners, reportStates,
  type ReportDetail, type ReportFilters, type ReportPage, type ReportReview, type ReportWorkflow,
  type SourceSystemOption, type WorkflowApprover, type WorkflowSigner,
...
export function AdminReportsPanel() {
  ...
  const [workflow, setWorkflow] = useState<ReportWorkflow>();
  ...
  useEffect(() => {
    Promise.all([fetchReports(emptyReportFilters()), fetchReportSourceSystems(), fetchWorkflowSigners(), fetchWorkflowApprovers()])
      .then(([page, loadedSources, loadedSigners, loadedApprovers]) => { setResult(page); setSources(loadedSources); setSigners(loadedSigners); setApprovers(loadedApprovers); })
      .catch((reason: Error) => setError(reason.message));
  }, []);

  async function search(event?: FormEvent, nextPage = 0, nextFilters = filters) {
    event?.preventDefault(); setError(undefined); setDetail(undefined); setWorkflow(undefined); setReview(undefined);
  ...
  async function showDetail(id: string) {
    setError(undefined);
    try { const [loadedDetail, loadedWorkflow, loadedReview] = await Promise.all([fetchReportDetail(id), fetchReportWorkflow(id), fetchReportReview(id)]); setDetail(loadedDetail); setWorkflow(loadedWorkflow); setReview(loadedReview); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "Dettaglio non disponibile"); }
  }

  async function refreshDetail(id: string) {
    const [loadedDetail, loadedWorkflow, loadedReview, page] = await Promise.all([
      fetchReportDetail(id), fetchReportWorkflow(id), fetchReportReview(id), fetchReports(applied, result.page),
    ]);
    setDetail(loadedDetail); setWorkflow(loadedWorkflow); setReview(loadedReview); setResult(page);
  }
  ...
  return <div className="reports-layout">
    ...
    {detail && workflow && review ? <ReportDetailPanel detail={detail} workflow={workflow} review={review} signers={signers} approvers={approvers}
      reload={() => refreshDetail(detail.id)} close={() => { setDetail(undefined); setWorkflow(undefined); setReview(undefined); }} /> : null}
  </div>;
}
...
function ReportDetailPanel({ detail, workflow, review, signers, approvers, reload, close }: { detail: ReportDetail; workflow: ReportWorkflow; review: ReportReview; signers: WorkflowSigner[]; approvers: WorkflowApprover[]; reload: () => Promise<void>; close: () => void }) {
  const { text } = useUiTexts();
