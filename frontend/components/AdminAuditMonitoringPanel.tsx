  ...
  return <div className="audit-layout">
    <section className="card audit-filter-card">
      ...
      <form className="audit-filter-form" onSubmit={(event) => search(event)}>
        ...
        <label>Entità<select value={filters.entityType} onChange={(e) => setFilters({ ...filters, entityType: e.target.value })}><option value="">Tutte</option><option>REPORT</option><option>DOCUMENT</option><option>HL7_MESSAGE</option><option>SIGNATURE_BATCH</option><option>SIGNATURE_ATTEMPT</option><option>CONFIGURATION</option><option>SESSION</option></select></label>
