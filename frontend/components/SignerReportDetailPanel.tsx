...
export function SignerReportDetailPanel({ reportId }: { reportId: string }) {
  ...
  return <div className="signer-layout"><section className="card signer-wide"><div className="section-title"><div><h1>Referto {report.internalIdentifier}</h1><p>Pratica {report.practice.practiceIdentifier}</p></div><Link className="link-button" href="/firma/referti">{text("button.backToReports")}</Link></div>
    ...
    <section className="card signer-wide mock-signature-card"><h2>Firma singola mock</h2>
      ...
      <p>{report.signatureEligible ? "Il referto è idoneo alla firma secondo le regole della sua tipologia." : "Il referto non soddisfa ancora le regole della sua tipologia per la firma."}</p>
      <form className="provider-session-form" onSubmit={authenticate}><label>Codice autorizzazione mock<input required value={authCode} onChange={(event) => setAuthCode(event.target.value)} inputMode="numeric" /></label><button disabled={opening || !report.signatureEligible}>{text("button.startProviderSession")}</button></form>
      {session ? <div className="batch-actions"><span className="status">Sessione attiva fino alle {formatDate(session.expiresAt)}</span><button className="primary" disabled={opening || !report.signatureEligible} onClick={sign}>{text("button.signMockSingle")}</button></div> : null}
