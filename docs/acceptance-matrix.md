...
| Users, roles, groups, partitions, companies | Goal 2 / Goal 3 | Implemented for local MVP administration | Identity model, admin APIs, Keycloak demo users | Admin can manage users, active state, roles, groups, partition, company, OIDC subject, fiscal code, contact data, and counter-signer fiscal-code field; signer receives 403 on admin APIs. | Full IAM federation and SAC production integration. |
